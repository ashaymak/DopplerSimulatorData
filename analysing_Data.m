...
%% Load golf shot datasets

Fs=17000;   % Sampling rate
...
l=0.02885; % wavelength of transmitted signal (2.885 cm)
%% Time Domain of Data
figure('Color',[1 1 1]);
subplot(3,1,1)
...
overlap = wlen*0.5;                 %percent overlap                    
nfft = wlen;                        %number of dft points                
...
overlap = wlen*0.98;                    %percent overlap                    
nfft = wlen;                            %number of dft points                
...
wlen =400;                              %length of window                 
overlap = wlen*0.98;                    %percent overlap                    
nfft = wlen;                            %number of fft points                
win = kaiser(wlen,15);                  %specifies type of window
Fs=17000;
...
overlap = wlen*0.98;                    %98 percent overlap                    
nfft = wlen;                            %fft length              
win = kaiser(wlen,50);                  %specifies type of window
Fs=17000;


figure('Color',[1 1 1]);
[s,f,t,p]=spectrogram(DriverDownSwing,win,overlap,nfft,Fs,'MinThreshold',65,'twosided'); 
view(0,90);colormap jet;
v=(flip(-f)*0.02855)/2;
P=10*log10(abs(p));
